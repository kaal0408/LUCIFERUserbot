...
from ..helpers.utils import _format, _lucifertools, _luciferutils, install_pip, reply_id
...
def load_module(shortname, plugin_path=None):
    if shortname.startswith("__"):
        ...
        mod.Config = Config
        mod._format = _format
        mod.tgbot = luciferub.tgbot
        mod.sudo_cmd = sudo_cmd
        mod.CMD_HELP = CMD_HELP
        mod.reply_id = reply_id
        mod.admin_cmd = admin_cmd
        mod._luciferutils = _luciferutils
        mod._lucifertools = _lucifertools
...
def remove_plugin(shortname):
        ...
        for cmdname in cmd:
            if cmdname in LOADED_CMDS:
                for i in LOADED_CMDS[cmdname]:
                    luciferub.remove_event_handler(i)
    ...
    try:
        name = f"userbot.plugins.{shortname}"
        for i in reversed(range(len(luciferub._event_builders))):
            ev, cb = luciferub._event_builders[i]
            if cb.__module__ == name:
                del luciferub._event_builders[i]
    except BaseException:
        raise ValueError
