    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.COMMAND_HAND_LER) == 2:
                luciferreg = "^" + Config.COMMAND_HAND_LER
                reg = Config.COMMAND_HAND_LER[1]
            elif len(Config.COMMAND_HAND_LER) == 1:
                luciferreg = "^\\" + Config.COMMAND_HAND_LER
                reg = Config.COMMAND_HAND_LER
            args["pattern"] = re.compile(luciferreg + pattern)
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.SUDO_COMMAND_HAND_LER) == 2:
                luciferreg = "^" + Config.SUDO_COMMAND_HAND_LER
...
def errors_handler(func):
    async def wrapper(errors):
        try:
            ...
            link = "[here](https://t.me/luciferXsupport)"
...
def register(**args):
    ...
    def decorator(func):
        if not disable_edited:
            luciferub.add_event_handler(func, MessageEdited(**args))
        luciferub.add_event_handler(func, NewMessage(**args))
        try:
