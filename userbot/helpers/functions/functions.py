...
async def hide_inlinebot(borg, bot_name, text, chat_id, reply_to_id, c_lick=0):
    ...
    lucifer = await sticcers[c_lick].click("me", hide_via=True)
    if lucifer:
        await borg.send_file(int(chat_id), lucifer, reply_to=reply_to_id)
...
async def waifutxt(text, chat_id, reply_to_id, bot):
    ...
    lucifer = await sticcers[0].click("me", hide_via=True)
    if lucifer:
        await bot.send_file(int(chat_id), lucifer, reply_to=reply_to_id)
        await lucifer.delete()
