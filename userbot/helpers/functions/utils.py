...
async def admin_groups(luciferub):
    lucifergroups = []
    async for dialog in luciferub.iter_dialogs():
        ...
        if (
            ...
            lucifergroups.append(entity.id)
    return lucifergroups
...
async def extract_time(lucifer, time_val):
    if any(time_val.endswith(unit) for unit in ("s", "m", "h", "d", "w")):
        unit = time_val[-1]
        time_num = time_val[:-1]  # type: str
        if not time_num.isdigit():
            await lucifer.edit("Invalid time amount specified.")
            return None
        if unit == "s":
