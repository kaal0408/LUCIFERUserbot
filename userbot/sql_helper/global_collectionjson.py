...
class Lucifer_GlobalCollection_Json(BASE):
...
Lucifer_GlobalCollection_Json.__table__.create(checkfirst=True)


def get_collection(keywoard):
    try:
        return SESSION.query(Lucifer_GlobalCollection_Json).get(keywoard)
    finally:
        SESSION.close()
...
def add_collection(keywoard, json, njson={}):
    ...
    if to_check:
        keyword_items = SESSION.query(Lucifer_GlobalCollection_Json).get(keywoard)
        SESSION.delete(keyword_items)
    keyword_items = Lucifer_GlobalCollection_Json(keywoard, json, njson)
...
def get_collections():
    try:
        return SESSION.query(Lucifer_GlobalCollection_Json).all()
    finally:
        SESSION.close()
