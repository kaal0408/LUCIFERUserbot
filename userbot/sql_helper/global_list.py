...
class LuciferGloballist(BASE):
    __tablename__ = "Luciferglobal_list"
    ...
    def __eq__(self, other):
        return bool(
            isinstance(other, LuciferGloballist)
            and self.keywoard == other.keywoard
            and self.group_id == other.group_id
        )


LuciferGloballist.__table__.create(checkfirst=True)

LuciferGLOBALLIST_INSERTION_LOCK = threading.RLock()
...
def add_to_list(keywoard, group_id):
    with LuciferGLOBALLIST_INSERTION_LOCK:
        broadcast_group = LuciferGloballist(keywoard, str(group_id))
...
def rm_from_list(keywoard, group_id):
    with LuciferGLOBALLIST_INSERTION_LOCK:
        broadcast_group = SESSION.query(LuciferGloballist).get((keywoard, str(group_id)))
        if broadcast_group:
...
def del_keyword_list(keywoard):
    with LuciferGLOBALLIST_INSERTION_LOCK:
        broadcast_group = (
            SESSION.query(LuciferGloballist.keywoard)
            .filter(LuciferGloballist.keywoard == keywoard)
            .delete()
        )
...
def get_list_keywords():
    try:
        chats = SESSION.query(LuciferGloballist.keywoard).distinct().all()
...
def num_list():
    try:
        return SESSION.query(LuciferGloballist).count()
    finally:
        SESSION.close()
...
def num_list_keywords():
    try:
        return SESSION.query(func.count(distinct(LuciferGloballist.keywoard))).scalar()
    finally:
        SESSION.close()
...
def __load_chat_lists():
    try:
        ...
        all_groups = SESSION.query(LuciferGloballist).all()
        for x in all_groups:
