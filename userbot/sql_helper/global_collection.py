...
class LUCIFER_GlobalCollection(BASE):
    __tablename__ = "LuciferX_globalcollection"
    ...
    def __eq__(self, other):
        return bool(
            isinstance(other, Lucifer_GlobalCollection)
            and self.keywoard == other.keywoard
            and self.contents == other.contents
        )


Lucifer_GlobalCollection.__table__.create(checkfirst=True)

LUCIFER_GLOBALCOLLECTION = threading.RLock()
...
def add_to_collectionlist(keywoard, contents):
    with LUCIFER_GLOBALCOLLECTION:
        keyword_items = LUCIFER_GlobalCollection(keywoard, tuple(contents))
...
def rm_from_collectionlist(keywoard, contents):
    with LUCIFER_GLOBALCOLLECTION:
        keyword_items = SESSION.query(LUCIFER_GlobalCollection).get(
...
def del_keyword_collectionlist(keywoard):
    with LUCIFER_GLOBALCOLLECTION:
        keyword_items = (
            SESSION.query(LUCIFER_GlobalCollection.keywoard)
            .filter(LUCIFER_GlobalCollection.keywoard == keywoard)
            .delete()
        )
...
def get_collectionlist_items():
    try:
        chats = SESSION.query(LUCIFER_GlobalCollection.keywoard).distinct().all()
...
def num_collectionlist():
    try:
        return SESSION.query(LUCIFER_GlobalCollection).count()
    finally:
        SESSION.close()
...
def num_collectionlist_items():
    try:
        return SESSION.query(
            func.count(distinct(LUCIFER_GlobalCollection.keywoard))
        ).scalar()
    finally:
        SESSION.close()
...
def __load_item_collectionlists():
    try:
        ...
        all_groups = SESSION.query(LUCIFER_GlobalCollection).all()
        for x in all_groups:
