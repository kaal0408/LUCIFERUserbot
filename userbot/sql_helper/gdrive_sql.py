...
class Gdrive(BASE):
    __tablename__ = "lucifergdrive"
    lucifer = Column(String(50), primary_key=True)

    def __init__(self, lucifer):
        self.lucifer = lucifer
...
def is_folder(folder_id):
    try:
        return SESSION.query(Gdrive).filter(Gdrive.lucifer == str(folder_id))
...
def rmparent_id(folder_id):
    note = SESSION.query(Gdrive).filter(Gdrive.lucifer == folder_id)
    if note:
        note.delete()
        SESSION.commit()
