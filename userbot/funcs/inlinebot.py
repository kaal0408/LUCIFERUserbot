...
luciferLOGO = "https://telegra.ph/file/9c1bcdcb9185cbe7d01ab.jpg"
...
@luciferub.tgbot.on(InlineQuery)
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**luciferXub"):
            buttons = [
                (
                    Button.inline("Stats", data="stats"),
                    Button.url("Repo", "https://github.com/TeamluciferX/luciferX"),
                )
            ]
            ALIVE_PIC = gvarstatus("ALIVE_PIC")
            IALIVE_PIC = gvarstatus("IALIVE_PIC")
            if IALIVE_PIC:
                lucifer = [x for x in IALIVE_PIC.split()]
                PIC = list(lucifer)
                I_IMG = random.choice(PIC)
            if not IALIVE_PIC and ALIVE_PIC:
                lucifer = [x for x in ALIVE_PIC.split()]
                PIC = list(lucifer)
                I_IMG = random.choice(PIC)
            elif not IALIVE_PIC:
                I_IMG = None
            ...
            result = builder.article(
                title="© luciferX Help",
                description="Help menu for luciferX",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
            ...
            if PM_PIC:
                lucifer = [x for x in PM_PIC.split()]
                PIC = list(lucifer)
                lucifer_IMG = random.choice(PIC)
            else:
                lucifer_IMG = None
            query = gvarstatus("pmpermit_text")
            if lucifer_IMG and lucifer_IMG.endswith((".jpg", ".jpeg", ".png")):
                result = builder.photo(
                    lucifer_IMG,
                    # title="Alive lucifer",
                    text=query,
        ...
        buttons = [
            (
                Button.url("Source code", "https://github.com/kaal0408/LuciferUserbot"),
                Button.url(
                    "Deploy",
                    "https://dashboard.heroku.com/new?button-url=https%3A%2F%2Fgithub.com%2Fkaal0408%2FLuciferUserbor&template=https%3A%2F%2Fgithub.com%2Fkaal0408%2FLuciferUserbot",
                ),
            )
        ]
        markup = event.client.build_reply_markup(buttons)
        photo = types.InputWebDocument(
            url=luciferLOGO, size=0, mime_type="image/jpeg", attributes=[]
        )
        text, msg_entities = await event.client._parse_message_text(
            "𝗗𝗲𝗽𝗹𝗼𝘆 𝘆𝗼𝘂𝗿 𝗼𝘄𝗻 L U C I F E R 𝗨𝘀𝗲𝗿𝗯𝗼𝘁.", "md"
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
            type="photo",
            title="L U C I F E R 𝙐𝙨𝙚𝙧𝙗𝙤𝙩",
            description="Deploy yourself",
            url="https://github.com/kaal0407/LuciferUserbot",
            thumb=photo,
            content=photo,
            send_message=types.InputBotInlineMessageMediaAuto(
                reply_markup=markup, message=text, entities=msg_entities
...
@luciferub.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@luciferub.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@luciferub.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@luciferub.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
