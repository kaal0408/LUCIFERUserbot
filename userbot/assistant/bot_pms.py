...
from userbot import Config, luciferub
    ...
    if chat.id != Config.OWNER_ID:
        ...
        if customstrmsg is not None:
            ...
            start_msg = f"Hey! 👤{mention},\
                        ...
                        \n\nPowered by [Luciferub](https://t.me/Lucifer_support_group)"
        buttons = [
            (
                Button.url("Repo", "https://github.com/kaal0408/LuciferUserbot"),
                Button.url(
                    "Deploy",
                    "https://dashboard.heroku.com/new?button-url=https%3A%2F%2Fgithub.com%2Fkaal04082FLuciferUserbot&template=https%3A%2F%2Fgithub.com%2Fkaal0408%2FLuciferUserbot",
                ),
            )
        ]
    else:
...
@luciferub.bot_cmd(incoming=True, func=lambda e: e.is_private)
...
@luciferub.bot_cmd(pattern="^/uinfo$", from_users=Config.OWNER_ID)
async def bot_start(event):
    reply_to = await reply_id(event)
    if not reply_to:
...
async def send_flood_alert(user_) -> None:
    ...
    if FloodConfig.ALERT and (user_.id in FloodConfig.ALERT.keys()):
        ...
        try:
            ...
            if BOTLOG:
                await luciferub.tgbot.send_message(
                    BOTLOG_CHATID,
    ...
    if found:
        if flood_count >= FloodConfig.AUTOBAN:
            if user_.id in Config.SUDO_USERS:
                ...
                if BOTLOG:
                    await luciferub.tgbot.send_message(BOTLOG_CHATID, sudo_spam)
            else:
        ...
        if not fa_id:
            return
        try:
            msg_ = await luciferub.tgbot.get_messages(BOTLOG_CHATID, fa_id)
        ...
        if BOTLOG:
            fa_msg = await luciferub.tgbot.send_message(
                BOTLOG_CHATID,
                flood_msg,
                buttons=buttons,
            )
        try:
            chat = await luciferub.tgbot.get_entity(BOTLOG_CHATID)
            await luciferub.tgbot.send_message(
                Config.OWNER_ID,
            ...
            if BOTLOG:
                await lucifetub.tgbot.send_message(BOTLOG_CHATID, "**Unblock your bot !**")
    if FloodConfig.ALERT[user_.id].get("fa_id") is None and fa_msg:
        FloodConfig.ALERT[user_.id]["fa_id"] = fa_msg.id


@luciferub.tgbot.on(CallbackQuery(data=re.compile(b"bot_pm_ban_([0-9]+)")))
@check_owner
async def bot_pm_ban_cb(c_q: CallbackQuery):
    user_id = int(c_q.pattern_match.group(1))
    try:
        user = await luciferub.get_entity(user_id)
    except Exception as e:
...
@luciferub.tgbot.on(CallbackQuery(data=re.compile(b"toggle_bot-antiflood_off$")))
@check_owner
...
@luciferub.bot_cmd(incoming=True, func=lambda e: e.is_private)
@luciferub.bot_cmd(edited=True, func=lambda e: e.is_private)
