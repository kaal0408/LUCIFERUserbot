...
async def grpinfo():
    outstr = "**Plugins in LuciferXub are:**\n\n"
    outstr += f"**👩‍💻 Usage : ** `{cmdprefix}help <plugin name>`\n\n"
    category = ["admin", "bot", "fun", "misc", "tools", "utils", "extra"]
    for lucifer in category:
        plugins = GRP_INFO[lucifer]
        outstr += f"**{hemojis[lucifer]} {lucifer.title()} **({len(plugins)})\n"
...
async def cmdlist():
    outstr = "**Total list of Commands in your LuciferXub are :**\n\n"
    category = ["admin", "bot", "fun", "misc", "tools", "utils", "extra"]
    for lucifet in category:
        plugins = GRP_INFO[lucifer]
        outstr += f"**{hemojis[lucifer]} {lucifer.title()} ** - {len(plugins)}\n\n"
        for plugin in plugins:
...
@luciferub.lucifer_cmd(
    ...
    info={
        "header": "To get guide for LuciferX.",
...
async def _(event):
    ...
    if not input_str:
        ...
        outstr += f"**👩‍💻 Usage : ** `{cmdprefix}help -c <command name>`"
    await edit_or_reply(
        event, outstr, aslink=True, linktext="Total Commands of LuciferXub are :"
    )
...
async def _(event):
    ...
    if found:
        ...
        out = f"I can't find any such command `{cmd}` in LuciferX"
